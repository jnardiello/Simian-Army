    ...
    public function addReviewToAsin($review)
    {
        ...
        if (empty($doc)) {
            $this->reviewsCollection->insert($review);
             $domain = "simian.army";
             $html = "<h1>A new review was added</h1><br /><div>Title: {$review['review_title']}<br />Author: {$review['review_author']}<br />Product: <a href='http://www.amazon.co.uk/dp/{$review['asin']}'>{$review['asin']}</a><br /><b>Review:</b><br />{$review['text']}<br /><a href='{$review['permalink']}'>Link</a></div>";
